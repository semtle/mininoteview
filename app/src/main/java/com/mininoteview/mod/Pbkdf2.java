...
class Pbkdf2
{
	private static final int DKLENMAX = 1024 * 1024;
	private static final int COUNTMAX = 1024 * 1024;
	...
	static byte[] HmacMD5(byte[] P, byte[] S, int c, int dkLen)
	{
		return pbkdf2("HmacMD5", P, S, c, dkLen);
	}

	static byte[] HmacSHA1(byte[] P, byte[] S, int c, int dkLen)
	{
		return pbkdf2("HmacSHA1", P, S, c, dkLen);
	}

	static byte[] HmacSHA256(byte[] P, byte[] S, int c, int dkLen)
	{
		return pbkdf2("HmacSHA256", P, S, c, dkLen);
	}

	static byte[] HmacSHA384(byte[] P, byte[] S, int c, int dkLen)
	{
		return pbkdf2("HmacSHA384", P, S, c, dkLen);
	}

	static byte[] HmacSHA512(byte[] P, byte[] S, int c, int dkLen)
	{
